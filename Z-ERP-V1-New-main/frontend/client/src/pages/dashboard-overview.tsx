...
export default function DashboardOverview() {
  ...
  const [kpiCounts, setKpiCounts] = useState({ projects: 0, tasks: 0, isLoading: true });
  ...
  useEffect(() => {
    ...
    const fetchKpiCounts = async () => {
      try {
        ...
        if (!active) return;
        console.warn('Failed to load KPI counts from Supabase', error);
        setKpiCounts((prev) => ({ ...prev, isLoading: false }));
      }
    };
  ...
  const kpiStats = useMemo(() => [
    {
      title: 'Projects',
      value: kpiCounts.isLoading ? '—' : String(kpiCounts.projects),
      icon: Target,
      change: 'Live',
      trend: 'up',
    ...
    {
      title: 'Tasks',
      value: kpiCounts.isLoading ? '—' : String(kpiCounts.tasks),
      icon: CheckSquare,
      change: 'Live',
      trend: 'up',
      subtitle: 'from Supabase'
