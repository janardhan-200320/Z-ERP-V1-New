...
export default function HRMEmployees() {
  ...
  const handleUpdateEmployee = async () => {
    ...
    try {
      ...
      const payload = {
        full_name: String(editingEmployee.name).trim(),
        designation: String(editingEmployee.designation).trim(),
        department: editingEmployee.department,
        location: String(editingEmployee.location ?? '').trim(),
        gender: String(editingEmployee.gender ?? '') || null,
        email: String(editingEmployee.email ?? '').trim().toLowerCase(),
        phone: String(editingEmployee.phone ?? '').trim(),
        bank_name: String(editingEmployee.bankName ?? '').trim(),
        bank_account_number: String(editingEmployee.accountNumber ?? '').trim(),
        bank_routing_number: String(editingEmployee.ifscCode ?? '').trim(),
        join_date: editingEmployee.joining,
      };

      const response = await fetch(`${apiUrl}/hrm/employees/${editingEmployee.id}`, {
        method: 'PUT',
      ...
      toast({
        ...
        description: `${editingEmployee.name}'s information has been saved to the database.`,
      });
    } catch (error: any) {
  ...
  // Employee data will be populated from HRM Context
  ...
  const filteredEmployees = useMemo(() => {
    return employees.filter(emp => {
      ...
      const matchesSearch = 
        ...
        empDesignation.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const handleAddEmployee = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        bank_name: newEmployee.bankName.trim(),
        bank_account_number: newEmployee.accountNumber.trim(),
        bank_routing_number: newEmployee.ifscCode.trim(),
      };
      ...
      // Normalize the created employee to match frontend structure
      const normalizedEmployee = {
        ...createdEmployee,
        name: createdEmployee.name || `${createdEmployee.first_name || ''} ${createdEmployee.last_name || ''}`.trim(),
        designation: createdEmployee.designation || payload.designation,
        department: createdEmployee.department || payload.department,
        email: createdEmployee.email || payload.email,
        phone: createdEmployee.phone || payload.phone,
        status: createdEmployee.status || 'onboarding'
      };
      ...
      setNewEmployee({
        name: '', designation: '', department: '', location: '', gender: '',
        employeeType: '', customEmployeeType: '', probationPeriod: '90',
        customProbationDays: '', personalEmail: '', email: '', officialPhone: '',
        phone: '', alternatePhone: '', bloodGroup: '',
        joining: new Date().toISOString().split('T')[0], photo: null,
        bankName: '', bankBranch: '', accountNumber: '', ifscCode: '',
        panNumber: '', aadhaarNumber: '', aadhaarDoc: null, panDoc: null, resume: null
      });

      // Refetch all employees to ensure database persistence
      setTimeout(() => {
        refetchEmployees();
      }, 500);
      ...
      toast({ 
        ...
        description: `${newEmployee.name} has been successfully added to the database.`
      });
    } catch (error: any) {
      console.error('Error creating employee:', error);
      
      // Parse error message for better UX
      let errorDescription = error.message || "Failed to create employee";
      
      if (error.message?.includes('duplicate') || error.message?.includes('already exists')) {
        if (error.message?.includes('email')) {
          errorDescription = `An employee with email "${newEmployee.email}" already exists in the database. Please use a different email address.`;
        } else {
          errorDescription = "This record already exists. Please check the information and try again.";
        }
      } else if (error.message?.includes('not found')) {
        errorDescription = "The department or designation was not found. Please try creating with different values.";
      }
  ...
  const handleUpdateStatus = async (employeeId: string, newStatus: string, workflow?: any) => {
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5000/api';
      const response = await fetch(`${apiUrl}/hrm/employees/${employeeId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || errorData.message || 'Failed to update employee status');
      }
  ...
  const handleDeleteEmployee = async (employee: any) => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5000/api';
      const response = await fetch(`${apiUrl}/hrm/employees/${employee.id}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' }
      });
                            ...
                            <div className="flex items-center justify-between">
                              <div className="min-w-0 flex-1">
                                ...
                                <h3 className="text-2xl sm:text-3xl font-black text-slate-700">
                                  {filteredEmployees.filter(emp => emp.status === 'exit').reduce((acc, emp) => acc + (emp.exitWorkflow?.assets?.filter((asset: any) => asset.status === 'pending_return').length || 0), 0)}
                                </h3>
          ...
          <div className="grid gap-4 py-4">
            ...
            <div>
              ...
              <Select onValueChange={(value) => setInterviewFormData({...interviewFormData, interviewer: value})}>
                ...
                <SelectContent>
                  {/* Interviewers will be populated from employees list */}
                </SelectContent>
              </Select>
            </div>
            <div>
            ...
            <DropdownMenuContent align="end" className="w-56 rounded-xl">
              ...
              <DropdownMenuSub>
                ...
                <DropdownMenuSubContent className="w-56 rounded-xl">
                  <DropdownMenuItem onClick={() => onUpdateStatus(employee.id, 'active')} className="cursor-pointer">
                    <UserCheck className="h-4 w-4 mr-2 text-emerald-600" />
                    Move to Active
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onUpdateStatus(employee.id, 'probation')} className="cursor-pointer">
                    <Clock className="h-4 w-4 mr-2 text-amber-600" />
                    Move to Probation
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onUpdateStatus(employee.id, 'exit')} className="cursor-pointer">
                    <UserMinus className="h-4 w-4 mr-2 text-rose-600" />
                    Move to Exit Management
                  </DropdownMenuItem>
                </DropdownMenuSubContent>
              </DropdownMenuSub>
              <DropdownMenuItem
                onClick={() => onDelete(employee)}
                className="cursor-pointer text-rose-600 hover:text-rose-700 hover:bg-rose-50 font-bold"
              >
